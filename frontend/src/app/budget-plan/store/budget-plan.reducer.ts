...
const budgetPlanReducer = createReducer(
  ...
  on(loadActiveBudgetPlanSuccess, (state, { budgetPlan }) => {
    return {
      ...state,
      loadActiveBudgetPlanState: LoadingState.LOADED,
  ...
  on(loadActiveBudgetPlanFail, (state) => {
    return {
      ...state,
      loadActiveBudgetPlanState: { errorMsg: 'Failed to load active budget plan' },
    }
  }),
  ...
  on(BudgetPlanAction.loadBudgetPlanEntriesFail, (state, { error }) => {
    return {
      ...state,
      loadBudgetPlanEntriesState: { errorMsg: 'Failed to load budget plan entries' },
    }
  }),
  ...
  on(BudgetPlanAction.loadBudgetPlanReportFail, (state, { error }) => {
    return {
      ...state,
      loadBudgetPlanReportsState: { errorMsg: 'Failed to load budget plan report' },
    }
  }),
)
