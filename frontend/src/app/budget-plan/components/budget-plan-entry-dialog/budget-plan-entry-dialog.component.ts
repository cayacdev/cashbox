...
export class BudgetPlanEntryDialogComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.element = this.data.data as BudgetPlanEntry
    this.initForm()

    this.descriptions$ = this.store
      .select(selectCashBoxSettings)
      .pipe(map((state) => state.settings[this.data.cashBoxId]?.descriptions ?? []))
